...
**Last updated:** 2026-05-12 (Session 004)
...
|-----------|--------|-------|
| **Governance MCP Container** | `docker_architecture_chat.md` §409-676 | Pseudocode spec → implement. FastMCP+DockerSDK, SSE transport, tools: provision/destroy/list/modify agents. |
| **Agent Container Template** | `docker_architecture_chat.md` §680-706 | Dockerfile: `ghcr.io/astral-sh/uv` base + Flox apt install + PATH bypass. Single image for all agent types. |
...
| **ACP Protocol** | Not designed | `acp-sdk-python` wrappers for all downstream agents. TUI for user interaction. |
| **Dynamic Manifest / Hot-Reload** | `docker_architecture_chat.md` §714-907 | File-based manifest per agent subpath, watchdog-driven LangGraph recompilation. |

### 🟢 BUILT THIS SESSION
| Component | Location | Status |
|-----------|----------|--------|
| **Bootstrap Script** | `docs/plans/overhaul/bootstrap_project.sh` | Written, YAML validated, untested. Creates volumes + subdirectories + compose file. |
| **Docker Compose Template** | Generated by bootstrap script | YAML-validated. Correct volume mounts, subpaths, networks. Placeholder image: `langgraph-agent-base:latest`. |
...
| **ACP** | `acp-sdk-python` | Orchestrator ↔ Agents, User/TUI ↔ Agents (runtime) | Not built |
...
- User installs skills per project: `bunx skills add <name>` (writes to project_vol `shared/skills/`)
- Orchestrator manages per-agent skill allowlist in manifest.json
- Orchestrator symlinks allowed skills from `shared/skills/` into agent's subpath `skills/`
- Agent has `read_skill(name)` tool → reads from its local `skills/` directory
- Agent can request new skills → orchestrator bubbles to user → user approves → allowlist updated → symlink added
- Permissions: HITL gate for new skill requests, static allowlist otherwise
...
1. ACP specification — message format, capability registration, TUI
2. Orchestrator LangGraph stateflow — routing, lifecycle, RAG integration
3. Per-agent LangGraph graphs — only MM has one so far
4. Agent Dockerfile — Flox manifest contents, uv lock strategy, image size optimization
...
| Bootstrap script | `docs/plans/overhaul/bootstrap_project.sh` |
...
| Context (superseded by this doc) | `docs/context/` |
