...
def should_ping(model_data, context_tokens, time_since_last_msg_min, expected_away_time_min=60):
    """
    ...
    cost_ping = (ctx_millions * model_data["read"]) + ((10 / 1_000_000) * model_data["out"])
...
def send_silent_ping(session_id, model_name):
    ...
    payload = {
        ...
        "messages": [
            {"role": "system", "content": "[SYSTEM_SIGNAL]: CACHE_REFRESH_ONLY. DO NOT OUTPUT TEXT. ACKNOWLEDGE WITH A SINGLE PERIOD."}
        ],
        "max_tokens": 1
    }
    ...
    try:
        ...
        requests.post("https://opencode.ai/api/go/v1/chat/completions", json=payload, headers=headers)
        ...
        for session in sessions:
            ...
            if do_ping:
                ...
                if "TTL Safe" not in reason:
                    logging.debug(f"Session {session['id']} ({model_name}) ignored: {reason}")
