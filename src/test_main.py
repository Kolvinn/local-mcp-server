...
class TestFilterConstruction:
    ...
    def test_single_tag_filter(self):
        ...
        assert result == {
            "AND": [{"user_id": main.AGENT_ID}, {"tags": {"contains": "api"}}]
        }

    def test_multiple_tags_filter_uses_or_logic(self):
        """Multiple tags use OR logic, wrapped with user_id."""
        result = main.build_search_filters(
            ["api", "database"], None, None, main.AGENT_ID
        )
        assert result == {
            "AND": [
    ...
    def test_project_id_filter(self):
        ...
        assert result == {
            "AND": [{"user_id": main.AGENT_ID}, {"project_id": "my-project"}]
        }
    ...
    def test_source_user_filter(self):
        ...
        assert result == {"AND": [{"user_id": main.AGENT_ID}, {"source_user": "john"}]}
    ...
    def test_tags_project_user_combined(self):
        ...
        result = main.build_search_filters(
            ["tag1", "tag2"], "proj", "user", main.AGENT_ID
        )
...
class TestAddMemory:
    ...
    def test_add_memory_calls_client_with_correct_params(self):
        ...
        main.mem_client.add.return_value = {
            "results": [{"id": "abc-123", "memory": "test content"}]
        }
    ...
    def test_add_memory_with_related_files(self):
        ...
        main.add_memory(content="Test", related_files=related_files)
...
class TestSearchMemory:
    ...
    def test_search_builds_filters_correctly(self):
        ...
        with patch.object(main, "build_search_filters") as mock_build:
            ...
            main.search_memory(
                query="test", tags=["api"], project_id="test", source_user="alice"
            )

            mock_build.assert_called_once_with(
                ["api"], "test", "alice", user_id=main.AGENT_ID
            )
        ...
        main.mem_client.search.return_value = [
            {
                ...
                "metadata": {"project_id": "my-project", "tags": ["api", "test"]},
            }
        ]
...
class TestSyncMetadata:
    ...
    def test_sync_metadata_without_scope_summary(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                main.sync_metadata(
                    file_path=file_path, project_id="minimal-project", tags=[]
                )
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                nested_path = os.path.join(
                    tmpdir, "nested", "deep", ".memory-context.yaml"
                )

                result = main.sync_metadata(
                    file_path=nested_path, project_id="nested-project"
                )
    ...
    def test_sync_metadata_error_handling(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = main.sync_metadata(file_path=file_path, project_id="test")
    ...
    def test_sync_metadata_rejects_path_traversal(self):
        ...
        result = main.sync_metadata(
            file_path="../../etc/.memory-context.yaml", project_id="test"
        )
    ...
    def test_sync_metadata_accepts_valid_path(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                result = main.sync_metadata(
                    file_path=file_path, project_id="valid-project", tags=["test"]
                )
...
class TestListProjects:
    ...
    def test_list_projects_fallback_to_search(self):
        ...
        try:
            ...
            main.mem_client.search = MagicMock(
                return_value=[
...
class TestBuildGoalFilters:
    ...
    def test_empty_returns_user_id_only(self):
        ...
        result = main.build_goal_filters(
            None, None, None, None, None, None, None, main.AGENT_ID
        )
        assert result == {"user_id": main.AGENT_ID}
    ...
    def test_node_type_filter(self):
        ...
        result = main.build_goal_filters(
            "goal", None, None, None, None, None, None, main.AGENT_ID
        )
        assert result == {
            "AND": [{"user_id": main.AGENT_ID}, {"node_type": {"eq": "goal"}}]
        }

    def test_root_id_filter(self):
        """Root ID filter uses eq, wrapped with user_id."""
        result = main.build_goal_filters(
            None, "root-123", None, None, None, None, None, main.AGENT_ID
        )
        assert result == {
            "AND": [{"user_id": main.AGENT_ID}, {"root_id": {"eq": "root-123"}}]
        }

    def test_parent_id_filter(self):
        """Parent ID filter uses eq, wrapped with user_id."""
        result = main.build_goal_filters(
            None, None, "parent-456", None, None, None, None, main.AGENT_ID
        )
        assert result == {
            "AND": [{"user_id": main.AGENT_ID}, {"parent_id": {"eq": "parent-456"}}]
        }

    def test_session_id_filter(self):
        """Session ID filter uses eq, wrapped with user_id."""
        result = main.build_goal_filters(
            None, None, None, "session-1", None, None, None, main.AGENT_ID
        )
        assert result == {
            "AND": [{"user_id": main.AGENT_ID}, {"session_id": {"eq": "session-1"}}]
        }

    def test_status_filter(self):
        """Status filter uses eq, wrapped with user_id."""
        result = main.build_goal_filters(
            None, None, None, None, "active", None, None, main.AGENT_ID
        )
        assert result == {
            "AND": [{"user_id": main.AGENT_ID}, {"status": {"eq": "active"}}]
        }

    def test_project_id_filter(self):
        """Project ID filter uses eq, wrapped with user_id."""
        result = main.build_goal_filters(
            None, None, None, None, None, "proj-x", None, main.AGENT_ID
        )
        assert result == {
            "AND": [{"user_id": main.AGENT_ID}, {"project_id": {"eq": "proj-x"}}]
        }

    def test_single_tag_uses_contains(self):
        """Single tag creates contains filter, wrapped with user_id."""
        result = main.build_goal_filters(
            None, None, None, None, None, None, ["api"], main.AGENT_ID
        )
        assert result == {
            "AND": [{"user_id": main.AGENT_ID}, {"tags": {"contains": "api"}}]
        }
    ...
    def test_multiple_tags_use_or(self):
        """Multiple tags use OR logic, wrapped with user_id."""
        result = main.build_goal_filters(
            None, None, None, None, None, None, ["a", "b"], main.AGENT_ID
        )
        assert result == {
            "AND": [
                {"user_id": main.AGENT_ID},
    ...
    def test_tags_empty_list_ignored(self):
        ...
        result = main.build_goal_filters(
            None, None, None, None, None, None, [], main.AGENT_ID
        )
        assert result == {"user_id": main.AGENT_ID}
    ...
    def test_multiple_conditions_use_and(self):
        """Multiple conditions use AND logic with user_id."""
        result = main.build_goal_filters(
            "goal", "root-1", None, None, "active", None, None, main.AGENT_ID
        )
        assert result == {
            "AND": [
                {"user_id": main.AGENT_ID},
    ...
    def test_tag_plus_other_conditions_use_and(self):
        ...
        result = main.build_goal_filters(
            None, None, None, None, None, "proj-x", ["tag1"], main.AGENT_ID
        )
        assert "AND" in result
        assert len(result["AND"]) == 2
        assert result["AND"][0] == {"user_id": main.AGENT_ID}
    ...
    def test_goal_user_id_with_tags_and_project(self):
        """user_id + tags + project produces AND wrapping all three."""
        result = main.build_goal_filters(
            None, None, None, None, None, "proj-x", ["api"], main.AGENT_ID
        )
        assert result == {
            "AND": [
                {"user_id": main.AGENT_ID},
...
class TestAddGoalNode:
    ...
    def test_valid_call_builds_correct_metadata(self):
        ...
        assert metadata["node_id"] is not None  # App-level UUID stored in metadata
    ...
    def test_root_goal_sets_root_id_to_node_id(self):
        ...
        main.add_goal_node(main.AddGoalNodeInput(content="Root goal", node_type="goal"))
    ...
    def test_infer_defaults_to_false(self):
        ...
        main.add_goal_node(main.AddGoalNodeInput(content="Test", node_type="goal"))
...
class TestSearchGoalNodes:
    ...
    def test_user_id_in_filters(self):
        ...
        main.search_goal_nodes(main.SearchGoalNodesInput(query="test query"))
    ...
    def test_result_formatting(self):
        ...
        result = main.search_goal_nodes(main.SearchGoalNodesInput(query="deploy"))

        assert "Goal Node Search Results" in result
        assert (
            "goal-nod" in result
        )  # Truncated ID (8 chars): "goal-nod" from "goal-node-abc-def"
        assert "Score: 0.850" in result
    ...
    def test_empty_results(self):
        ...
        result = main.search_goal_nodes(main.SearchGoalNodesInput(query="nonexistent"))
    ...
    def test_error_handling(self):
        ...
        result = main.search_goal_nodes(main.SearchGoalNodesInput(query="test"))
...
class TestGetGoalTree:
    ...
    def test_get_all_called_with_correct_filters(self):
        ...
        assert call_args["filters"]["user_id"] == main.AGENT_ID
    ...
    def test_fallback_to_search(self):
        ...
        try:
            ...
            main.goal_client.search = MagicMock(
                return_value=[
                    {
                        "id": "root-1",
                        "memory": "Root",
...
class TestUpdateGoalNode:
    ...
    def test_partial_metadata_update(self):
        ...
        main.goal_client.search = MagicMock(
            return_value={"results": [{"id": "mem0-internal-id"}]}
        )
        ...
        # First positional arg is Mem0 internal ID (resolved from app-level node_id)
        assert call_args[0][0] == "mem0-internal-id"
        # data should not be in kwargs when content not provided (Fix 3)
    ...
    def test_update_with_content(self):
        ...
        main.goal_client.update.return_value = None
        main.goal_client.search.reset_mock()
        main.goal_client.search.return_value = {"results": [{"id": "mem0-internal-id"}]}
    ...
    def test_update_without_content_skips_data(self):
        """Verify data is not passed when content not provided (avoids embedder crash)."""
        main.goal_client.update.reset_mock()
        main.goal_client.update.return_value = None
        main.goal_client.search.reset_mock()
        main.goal_client.search.return_value = {"results": [{"id": "mem0-internal-id"}]}
        ...
        call_args = main.goal_client.update.call_args
        # data should not be passed at all (Fix 3: skip data param to avoid empty string crash)
        assert "data" not in call_args[1]
...
class TestDeleteGoalNode:
    ...
    def test_deletes_correct_node_id(self):
        ...
        main.goal_client.delete.assert_called_once_with("mem0-internal-id")
        assert "successfully" in result.lower()
    ...
    def test_error_handling(self):
        ...
        result = main.delete_goal_node(main.DeleteGoalNodeInput(node_id="invalid-id"))
...
class TestServerStartup:
    ...
    def test_main_block_runs_server(self, mock_run):
        ...
        mock_run.assert_called_once_with(
            transport="streamable-http", host=main.HOST, port=main.PORT
        )
